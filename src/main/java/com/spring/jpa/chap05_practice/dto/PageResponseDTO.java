...
public class PageResponseDTO<T> {
    ...
    // 제네릭 타입을 <T>로 선언하면 전달되는 객체의 제네릭 타입에 따라 T가 결정된다. -> 클래스에도 제네릭 타입 <T>를 선언해야 한다.
    public PageResponseDTO(Page<T> pageData) {
        ...
        this.prev = startPage > 1;  // startPage가 1보다 크다면 prev가 true
        this.next = endPage < realEnd; // endPage가 realEnd보다 작아야 next가 true
